...
def load_rgb_colmap(path):
    img = imageio.imread(path)[:, :, :3]
    img = np.float32(img)
    mask = imageio.imread(path)[:, :, 3:4]
...
def load_rgb_real_world(path, size=800):
    img = imageio.imread(path)[:, :, :3]
...
def load_mask_real_world(path, size=800):
    mask = imageio.imread(path)[:, :, :3]
...
def load_mask_nerd(path):
    # this is for nerd mask
    mask = pyexr.open(path).get()
    object_mask = mask > 0.5

    return object_mask

def load_mask(path):
    # this is for zhejiang mask
...
def lift(x, y, z, intrinsics):
    ...
    # homogeneous
    return torch.stack((x_lift, -y_lift, -z, torch.ones_like(z).cuda()), dim=-1) # synthesize
    # return torch.stack((x_lift, y_lift, z, torch.ones_like(z).cuda()), dim=-1) # DTU
