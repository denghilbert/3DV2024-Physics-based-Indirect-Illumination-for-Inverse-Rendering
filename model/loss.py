...
class InvLoss(nn.Module):
    def __init__(self, idr_rgb_weight, eikonal_weight, mask_weight, alpha,
                    sg_rgb_weight, kl_weight, latent_smooth_weight, comb_loss_weight,
        ...
        self.comb_loss_weight = comb_loss_weight
    ...
    def get_kl_loss(self, model, points, kl_output):

        loss = 0
        loss += self.kl_divergence(0.05, kl_output)

        return loss

    def forward(self, model_outputs, ground_truth, mat_model=None, train_idr=False, train_geometry=False):
        ...
        if train_idr:
            ...
            model_outputs['indir_rgb'][model_outputs['indir_rgb'] == 1] = 0
            ...
            kl_loss = self.get_kl_loss(mat_model, model_outputs['points'][network_object_mask], model_outputs['kl_output'])

            if model_outputs['comb'] != None:
                model_outputs['angle_reflectnormal_128'][model_outputs['angle_reflectnormal_128'] > 0] = 0
                comb_loss = model_outputs['comb'][model_outputs['angle_reflectnormal_128'].bool()].mean()
            else:
                comb_loss = torch.tensor([0.]).cuda()
            if torch.isnan(comb_loss):
                import pdb
                pdb.set_trace()


            rough_regu = (model_outputs['network_object_mask'].unsqueeze(-1)*model_outputs['roughness']).sum()/((model_outputs['network_object_mask']==True).sum()*3)
            ...
            loss = 0 * idr_rgb_loss + \
                   ...
                   self.comb_loss_weight * comb_loss
            ...
            output = {
                ...
                'comb_loss': comb_loss,
                'sg_specular_reflectance': model_outputs['sg_specular_reflectance'],
                'rough_regu': rough_regu,
                'sharpness': model_outputs['sharpness'],
                'amptitude': model_outputs['amptitude'],
                'loss': loss}
